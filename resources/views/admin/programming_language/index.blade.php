                ...
                <div class="card m-8 shadow-lg rounded-md px-8 pb-8">
                    <div class="flex justify-between">
                        <div class="flex items-center">
                            ...
                            <span
                                class="inline-flex ml-4 items-center justify-center px-2 py-1 mr-2 text-xs font-bold leading-none text-red-100 bg-indigo-600 rounded-full">{{ $languages->count() }}</span>
                        </div>

                        <a href="{{ route('admin.programming-language.create') }}"
                                            ...
                                            @foreach ($languages as $key => $language)
                                                <tr>
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        {{ $language->created_at->format('d/m/Y') }}</td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        <a href="{{ route('admin.programming-language.edit', $language->id) }}"
                                                            class="text-indigo-600 hover:text-indigo-900 mx-2">Edit</a>

                                                        <button class="text-red-600 hover:text-red-900" type="button"
                                                            onclick="deleteModel({{ $language->id }})">
                                                            Delete
                                                        </button>
                                                        <form id="delete-form-{{ $language->id }}"
                                                            action="{{ route('admin.programming-language.destroy', $language->id) }}"
